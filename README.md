...
- **Inbox with validation** — uploads are checked before they touch a game: IoStore pak sets (`.pak` + `.utoc` + `.ucas`) must be complete, broken or unsupported archives are listed with the reason instead of failing silently.
...
- **SD-card aware** — for games on a different filesystem, the mod store relocates next to that Steam library so enable/disable stays an atomic rename instead of a multi-GB copy.

## How it works

```
phone browser ──upload──▶ inbox ──assign──▶ mod store ──enable──▶ <Game>/Content/Paks/~mods
   (QR code)             (validated)      (per game)  ◀─disable──   (loaded by the engine)
```
...
3. **Upload** — a mod archive (`.zip` / `.7z`) or a bare `.pak`. A `.utoc`/`.ucas` cannot stand alone — send those inside an archive together with their matching `.pak`.
4. **Assign** — pick the game in the *Inbox*; the mod is imported and enabled immediately.
5. **Play** — launch the game. Toggle mods on/off any time; changes apply on next launch.

Mods live in a central store under `~/.local/share/moddock/` (or `<steam-library>/.moddock/` for games on another drive). Enabling moves the files into the game's `Paks/~mods`; disabling moves them back. No symlinks, no duplicate copies.
...
| Inbox entry marked "incomplete pak set" | The archive is missing one of `.pak`/`.utoc`/`.ucas` — re-download the mod. |
...
moddock/
  ...
  importer.py           Archive extraction, pak-set validation, inbox inspection
...
src/                    Quick Access panel (games / add / detail / inbox / settings)
```
